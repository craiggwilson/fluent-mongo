...
{
    [TestFixture]
    public class MapReduceTests : LinqTestBase
    {
        ...
        {
            var minAge = Collection.AsQueryable().Select(x => x.Age).Min();
        ...
        {
            var minAge = Collection.AsQueryable().Min(x => x.Age);
        ...
        {
            var grouping = Enumerable.ToList(from p in Collection.AsQueryable()
                                             where p.Age > 21
                                             group p by 1 into g
                                             select new
                                             {
